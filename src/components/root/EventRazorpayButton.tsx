...
export default function EventRazorpayButton({
  userId,
  eventId,
  price,
}: {
  ...
  const handlePayment = async () => {
    ...
    const response = await fetch("/api/payment/buy-event", {
      method: "POST",
    ...
    const options: any = {
      ...
      handler: async function (response: {
        ...
        const verifyResponse = await fetch("/api/payment/verify-event-payment", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
